...
var background = 'textures/sky.jpg';
var spriteImg = 'textures/sprite.png';
...
class Entity extends THREE.Object3D{
	...
	onOffLightCalc(materials) {
		...
		defmat = this.children[0].material;
	}
}
...
class Alien extends Entity {
	...
	treatCollision(obj){
		...
		//Alien-Spaceship collision should remove lives and end the game
		if(obj.myType() == "spaceship"){
			paused = true;
			--lives;
			if(!lives) {
				game_ended = true;
				...
				spritegroup.children[lives].visible = false;
				restartGame(lives);
			}
		}
	}
...
/*Creation of Aliens*/
function createAlien(alien, mat, x, y, z) {
...
function createRow(y, mat) {

	new Alien(15, y, 0, mat);
	new Alien(-15, y, 0, mat);
	new Alien(45, y, 0, mat);
	new Alien(-45, y, 0, mat);
	alive_aliens += 4;
}

function createAllAliens(defmat) {
	if (defmat) {
		createRow(25, defmat);
		createRow(10, defmat);
	}
...
function onResize() {
	...
	//OrthographicCamera
	if (window.innerWidth / window.innerHeight > ratio) {
		resizeHUDCamera(scale_height);
		resizeOrtCamera(scale_height);
	} else {
		resizeHUDCamera(scale_width);
...
function resizeHUDCamera(scale) {
	nCam.left = window.innerWidth / scale;
	nCam.right = -window.innerWidth / scale;
	nCam.top = window.innerHeight / scale;
	nCam.bottom = -window.innerHeight / scale;
...
function createSpotLight(){
	spotlight = new THREE.SpotLight(0xffffff,5,height/2 - 5);
...
/*Creates Invisible Pause Screen*/
...
/*Creates a Spaceship Sprite at (x,y,z) Position*/
function createSpaceshipSprite(x, y, z) {
...
/*Creates Invisible Restart Screen*/
...
function addHUD() {
	...
	// Spaceship Sprites
	createSpaceshipSprite(-70, 33, 0);
	createSpaceshipSprite(-65, 33, 0);
	createSpaceshipSprite(-60, 33, 0);
...
function createBackground() {
    var geometry = new THREE.PlaneGeometry(150, 70, 0);

	var texture = new THREE.TextureLoader().load(background);

	// Doesn't Stretch but instead Repeats Image for Better Quality Results
...
/*Restarts Game with X Amount of Lives Left (health)*/
...
/*Main Functions*/
function animate() {
	//Checks for keyboard input for movement if game is unpaused
	if(!paused) checkMove();
